...
class FittingMonitor():
    ...
    def run_fitting(self, optimizer, closure, params, body_models,
        ...
        for n in range(self.maxiters):
            ...
            savez.append(params[0].data.cpu().numpy()[0])
        
        np.savetxt('./contact_optim/log.txt', np.array(savez))
        return prev_loss
    ...
    def create_fitting_closure(self, optimizer, body_models, transl, pose, shape, joint2ds, regressor, intri,
        ...
        def fitting_func(backward=True):
            ...
            smooth_loss = torch.norm(transl[1:] - transl[:-1], dim=-1).mean() * 100
    ...
    def create_contact_closure(self, optimizer, model, up_sample_fn, up_sample_fn2, contact_2d, z, vertices_can_ds, loss, lamda=0.0, create_graph=False):
        def contact_func(backward=True):
            loss_dict = {}
            total_loss = 0.0
            if backward:
                optimizer.zero_grad()
            ...
            bceLoss = self.loss(gen_batch[0,:,0], contact_2d) #loss(gen_batch[0,:,0], contact_2d)
            ## l2loss = torch.einsum("ik,ik",[z,z])
            l2loss = torch.norm(z) * lamda
            total_loss += bceLoss
            total_loss += l2loss

            loss_dict['bceLoss'] = float(bceLoss.data)
            loss_dict['l2loss'] = float(l2loss.data)
